 ...
 *'''
   def colorToGrayscale(self, pel)
      ...
      lum = round(0.3 * float(rgb.) + 0.589 * float(rgb.green) + 0.11 * float(rgb.blue))
 ...
 *'''
   ...
   def main(String[] args)
      ...
      numberOfColors = None
      pel = None
      iByteVal, iColumn, iBytesPerRow, iPelsPerRow, iTrailingBits, iDeadBytes = None
      # RBGQUAD
      ...
      dibdumper = DibDump() # needed to get to the byte swapping methods
      ...
      try: # lots of things can go wrong when doing file i'''o
         ...
         *'''
         ...
         bmpFileHeader_bfType      = dibdumper.swapShort(struct.unpack(">H", fstream.read(2))[0])    # WORD
         bmpFileHeader_bfSize      = dibdumper.swapInt(struct.unpack(">i", fstream.read(4))[0])      # DWORD
         bmpFileHeader_bfReserved1 = dibdumper.swapShort(struct.unpack(">H", fstream.read(2))[0])    # WORD
         bmpFileHeader_bfReserved2 = dibdumper.swapShort(struct.unpack(">H", fstream.read(2))[0])    # WORD
         bmpFileHeader_bfOffBits   = dibdumper.swapInt(struct.unpack(">i", fstream.read(4))[0])      # DWORD

         print(
            str(bmpFileHeader_bfType) +
            str(bmpFileHeader_bfSize) +
            str(bmpFileHeader_bfReserved1) +
            str(bmpFileHeader_bfReserved2) +
            str(bmpFileHeader_bfOffBits)
         )
...
*'''
         ...
         # Read and convert to big endian
         dibdumper.swapInt(struct.unpack(">i", fstream.read(4))[0])                                    # DWORD
         dibdumper.swapInt(struct.unpack(">i", fstream.read(4))[0])                                    # LONG
         dibdumper.swapInt(struct.unpack(">i", fstream.read(4))[0])                                    # LONG
         bmpInfoHeader_biPlanes        = dibdumper.swapShort(struct.unpack(">H", fstream.read(2))[0])  # WORD
         bmpInfoHeader_biBitCount      = dibdumper.swapShort(struct.unpack(">H", fstream.read(2))[0])  # WORD
         dibdumper.swapInt(struct.unpack(">i", fstream.read(4))[0])                                    # DWORD
         dibdumper.swapInt(struct.unpack(">i", fstream.read(4))[0])                                    # DWORD
         dibdumper.swapInt(struct.unpack(">i", fstream.read(4))[0])                                    # LONG
         dibdumper.swapInt(struct.unpack(">i", fstream.read(4))[0])                                    # LONG
