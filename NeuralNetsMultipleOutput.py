...
def main(configFile): 
   ...
   for i in range(numInputs):
      ithInputFile = [str(val) for val in fileInputs.readline().split()][0]          #Parses ith input
      inputFile = open(ithInputFile, 'r')

      for j in range(layerSizes[0]):
         inputs[i][j] = [int(val) for val in inputFile.readline().split()][0] / inputRange
         #End of assigning inputs
         
      ithOutputFile = [str(val) for val in fileInputs.readline().split()][0]
      outputFile = open(ithOutputFile, 'r')

      for j in range(layerSizes[len(layerSizes) - 1]):
         expectedOutputs[i][j] = [int(val) for val in outputFile.readline().split()][0] / inputRange
